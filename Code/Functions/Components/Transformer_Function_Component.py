...
def Train_Transformer(train_data,
    ...
    try:
        ...
        weather_series = TimeSeries.from_dataframe(weather_data, time_col='ds', fill_missing_dates=True, freq="{minutes}T".format(minutes=diff_time))
...
def PredictFromTransformer(model,
                           input_data,
                           weather_data,
                           diff_time,
                           days_ahead=1):
    ...
    forecast = model.predict(n=24 * days_ahead * measures_per_hour, series=input_series_transformed, past_covariates=weather_transformed)
...
def TrainAndPredictTransformer(train_data, input_data, weather_data, diff_time, days_ahead):
    """
    ...
    model, metric = Train_Transformer(train_data, weather_data, diff_time=diff_time)
    preds_test = PredictFromTransformer(model, input_data, weather_data, diff_time, days_ahead)

    return model, preds_test

def LoadAndForecastTransformer(path_to_model, input_data, weather_data, diff_time, days_ahead):
    """
    ...
    forecast = PredictFromTransformer(model, input_data, weather_data, diff_time, days_ahead)
